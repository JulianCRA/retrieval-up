...
from compartido.utils import obtener_identificador, cronometro_activo, medir
    ...
    try:
        with cronometro_activo() as crono:
            with YoutubeDL(OPTS) as ydl:
                with medir("extraccion_metadatos"):
                ...
                with YoutubeDL(opts) as ydl2:
                    ...
                    if registrar_descarga(r_id, actual_uri, info):
                        registrar_detalles(r_id, actual_uri, info, archivo_descargado, tiempos=crono.resumen())
...
def registrar_detalles(r_id, uri, info, archivo_descargado, tiempos=None):
    archivo_detalle = DESCARGAS_DIR / r_id / "info.json"
